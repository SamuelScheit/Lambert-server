...
class Server {
	constructor(opts) {
		this.errorHandler = (error, req, res, next) => {
			try {
				...
				if (error instanceof HTTPError_1.HTTPError && error.code) code = error.code || 400;
				else {
					console.error(error);
				...
				return res
					.status(500)
					.json({ success: false, code: 500, error: true, message: "Internal Server Error" });
			}
		};
		if (!opts) opts = {};
	...
	registerRoute(root, file) {
		...
		try {
			...
			console.error(new Error(`[Server] Failed to register route ${path}: ${error}`), error);
		}
	}
	stop() {
